...
const MainPageContent = () => {
  ...
  const [currentArtilleryImageIndex, setCurrentArtilleryImageIndex] =
    useState(0);
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      setCurrentAuctionImageIndex(
        (prevIndex) => (prevIndex + 1) % auctionImages.length
      );
    }, 4000);
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      setCurrentArtilleryImageIndex(
        (prevIndex) => (prevIndex + 1) % artilleryImages.length
      );
    }, 4000);
    ...
    <div className="min-h-screen bg-gray-900 text-white flex flex-col">
      ...
      <div className="relative">
        ...
        <section className="relative z-10 w-full bg-gray-900/60 text-white py-20">
          ...
          >
            ...
            <p className="text-xl text-gray-300 mb-10 max-w-md">{t("about")}</p>
      ...
      <section className="relative py-20 bg-gray-900 text-gray-100">
        ...
        >
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-10">
            {["trustedItems", "supportUkraine", "uniqueCollectibles"].map(
              (reasonKey, index) => (
                <motion.div
                  key={index}
                ...
                >
                  <h3 className="text-2xl font-semibold mb-4">
                    {t(`whyChooseUs.reasons.${reasonKey}Title`)}
                  </h3>
                  <p className="text-gray-400">
                    {t(`whyChooseUs.reasons.${reasonKey}`)}
                  </p>
                </motion.div>
              )
            )}
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-10">
            <div className="text-center">
              ...
              <h3 className="text-2xl font-semibold mt-4 mb-2">
                {t("howItWorks.steps.browseItemsTitle")}
              </h3>
              <p className="text-gray-400">
            ...
            <div className="text-center">
              ...
              <h3 className="text-2xl font-semibold mt-4 mb-2">
                {t("howItWorks.steps.placeBidTitle")}
              </h3>
              <p className="text-gray-400">{t("howItWorks.steps.placeBid")}</p>
            </div>
            <div className="text-center">
              <span className="text-5xl font-bold text-blue-500">3</span>
              <h3 className="text-2xl font-semibold mt-4 mb-2">
                {t("howItWorks.steps.makeImpactTitle")}
              </h3>
              <p className="text-gray-400">
        ...
        <div className="max-w-screen-lg mx-auto px-6 flex flex-col md:flex-row items-center md:space-x-10">
          ...
          >
            <div
              ...
              style={{
                transform: `translateX(-${currentAuctionImageIndex * 100}%)`,
              }}
            >
              ...
              {auctionImages.map((_, index) => (
                <div
                  ...
                  className={`w-3 h-3 rounded-full ${
                    index === currentAuctionImageIndex
                      ? "bg-white"
                      : "bg-gray-400"
                  }`}
                ></div>
      ...
      <section className="relative py-20 bg-gray-800 text-gray-100">
        ...
        <div className="max-w-screen-lg mx-auto px-6 flex flex-col md:flex-row items-center md:space-x-10">
          <div className="w-full md:w-1/2 mt-10 md:mt-0">
            <h3 className="text-3xl font-bold mb-4">
              {t("projectRevenge.title")}{" "}
              <span className="text-blue-500 text-4xl">"REVENGE"</span>
            </h3>
            ...
            <div className="absolute bottom-4 left-1/2 transform -translate-x-1/2 flex space-x-2">
              {artilleryImages.map((_, index) => (
                <div
                  key={index}
                  className={`w-3 h-3 rounded-full ${
                    index === currentArtilleryImageIndex
                      ? "bg-white"
                      : "bg-gray-400"
                  }`}
                ></div>
